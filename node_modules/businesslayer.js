...
class Player{
	...
	static connect(nick, login, password){
		Player.isConnected(nick, function(connected){
			if (connected){
				...
				db.get("SELECT rowid, * FROM Player WHERE login=$login AND password=$password",
					{$login: login, $password: md5(login+password)}, function(err, row){
					if (err){
						error(nick, err);
					}else if(row){
	...
	static disconnectAll(){
		db.run("UPDATE Player SET nick=null", function(){
			
		});
	}

	static isConnected(nick, callback){
		db.get("SELECT rowid, * FROM Player WHERE nick=?", nick, function(err, row){
			callback(!err && row ? true : false, row);
		})
	}
