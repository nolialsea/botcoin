...
const User = {
	...
	getByLogin: function(login, callback){
		db.get("SELECT rowid AS id, login, password, nick, gold, lastMining FROM User WHERE login=?", login, function(err, row) {
			callback(row);
		});
	},

	getByNick: function(nick, callback){
		db.get("SELECT rowid AS id, login, password, nick, gold, lastMining FROM User WHERE nick=?", nick, function(err, row) {
			callback(row);
		});
	},
