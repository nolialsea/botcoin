...
let command = function(commandName, callback){
	let reg = new RegExp("^"+commandName,"i");
					...
					if (user !== undefined){
						if (user.gold >= investment){
							...
							output(nick, "You created ["+name+"] ! Power : "+power.toFixed(8)+
								" ("+(randPower*100).toFixed(2)+"% success) | Max durability : "+
								durability.toFixed(8)+" ("+(randDurability*100).toFixed(2)+"% succes)");
						}else{
							...
							if (pickaxe){
								if (user.gold >= investment){
									...
									output(nick, 
										...
										" (+"+power.toFixed(8)+", or "+(randPower*100).toFixed(2)+
										"% success) | New max durability : "+(pickaxe.maxDurability+durability).toFixed(8)+
										" (+"+(randDurability*100).toFixed(2)+"% success)"
									);
								}else{
									output(nick, "You don't have enough gold");
								}
		...
		User.getByNick(nick, function(user){
			if (user === undefined){
				...
				if (res){
					const delta = Math.floor(Date.now()/1000) - user.lastMining;
					const rand = Math.random();
					const experience = rand*(delta/86400)*cfg.levelingRate/user.level;
