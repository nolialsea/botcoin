...
IRC bot that allows people to mine virtual, worthless gold, then use it (craft), trade it (for crafted stuff) or gamble it (because).
Players can create accounts and log in with PM command, they stay connected even when they change nick (store realname to stay connected ?)
Each player have a pickaxe they can use every X minute(s).

TODO :
    - Mining is scaled by delta (Max 1G/d ?)

POSSIBLE FUTURES :
	- Mining zones ! Mine the channel or BotCoin himself and get different outcomes
	- Pickaxes ! Craftable, upgradable(at some extent?), breakable, repairable(?)
	- TAX UPDATE ???
